...
毎回調べ直さなくて済むように、私が文書作成でよく使う LaTeX の小ワザを、コピーして試せる形でまとめます。
...
使用する機能に応じて、プリアンブルに次のパッケージを追加してください。
...
`\\` で強制改行するときは、直後の角括弧に長さを指定すると、通常の行間に縦方向の余白を追加できます。たとえば、`\\[0.1em]` は改行後に `0.1em` の余白を加えます。
...
この指定は `align`、`aligned`、`array`、`tabular` など、`\\` で行を区切る環境で利用できます。`em` は現在のフォントサイズを基準にする相対単位で、`pt` や `ex` なども指定できます。負の値を指定すると行間を狭められますが、文字や記号が重なる可能性があるため注意が必要です。

`\\[0.1em]` が指定するのは改行後に**追加する余白**であり、行間全体の大きさではありません。また、本文で新しい段落を始めたい場合は `\\` ではなく、空行または `\par` を使います。
